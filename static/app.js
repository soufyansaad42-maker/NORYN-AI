...
(() => {
  ...
  const state = { mode: "general", busy: false };
  ...
  async function sendMessage() {
    ...
    try {
      ...
      if (!contentType.includes("application/json")) {
        throw new Error("الخادم أعاد HTML بدل JSON. تأكد من مسار /api/chat في app.py.");
      }
      ...
      if (!answer) {
        throw new Error(data.error || data.detail || "لم نجد نص الإجابة.");
      }
    ...
    document.querySelectorAll("[data-mode]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        if (input) {
          ...
          input.placeholder = placeholders[state.mode] || "اكتب رسالتك...";
        }
      });
    });
  }
